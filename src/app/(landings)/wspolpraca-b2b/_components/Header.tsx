...
import { NAVY, MID, GRAD } from '../_lib/tokens';
    ...
    >
      <div className="max-w-7xl mx-auto px-5 lg:px-8 flex items-center justify-between h-16">
        ...
        <nav className="hidden lg:flex items-center gap-5">
          {navLinks.map(([label, id]) => (
            <button
              key={id}
              onClick={() => scrollTo(id)}
              className="text-sm font-medium transition-colors whitespace-nowrap hover:text-[#0F172A]"
              style={{ color: MID }}
            >
              {label}
            </button>
        ...
        <div className="flex items-center gap-3">
          <div ref={langRef} className="relative hidden sm:flex items-center gap-0.5 p-0.5 rounded-lg border" style={{ borderColor: 'rgba(37,99,235,0.15)', background: '#F1F5F9' }}>
            {(['PL', 'EN', 'RU', 'UA'] as Lang[]).map((l) => (
              <button
                key={l}
                onClick={() => setLang(l)}
                className="px-2.5 py-1 text-xs font-bold rounded-md transition-all"
                style={lang === l ? { background: GRAD, color: '#fff', boxShadow: '0 1px 6px rgba(37,99,235,0.3)' } : { color: MID }}
              >
                {l}
              </button>
            ))}
          ...
          <button
            ...
            className="hidden sm:flex items-center gap-1.5 text-sm font-semibold text-white px-4 py-2 rounded-xl transition-all hover:shadow-lg"
