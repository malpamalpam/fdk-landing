...
import { Code2, Palette, Compass, Camera, Video, Music2, Languages, BookOpen, PenLine, ShoppingCart, Newspaper, Target, XCircle } from 'lucide-react';
import { NAVY, BLUE, VIOLET, BG, MID } from '../_lib/tokens';
  ...
  return (
    <section id="industries" className="py-20 lg:py-28" style={{ background: BG, scrollMarginTop: '80px' }}>
      <div className="max-w-5xl mx-auto px-5 lg:px-8">
        ...
        <div className="mt-10 pt-8 border-t" style={{ borderColor: 'rgba(15,23,42,0.07)' }}>
          <p className="text-[11px] font-black uppercase tracking-[0.18em] mb-4" style={{ color: '#EF4444' }}>
            {t.industries.exclusionsHeading}
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {t.industries.exclusions.map((item, i) => (
              <div key={i} className="flex items-start gap-2.5 flex-1">
                ...
                <p className="text-xs leading-relaxed" style={{ color: MID }}>
                  <span className="font-semibold" style={{ color: '#991B1B' }}>{item.title}: </span>
                  {item.body}
                </p>
              </div>
            ))}
