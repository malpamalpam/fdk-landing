    ...
    <section id="how-it-works" className="py-20 lg:py-28" style={{ background: SLATE_1, scrollMarginTop: '80px' }}>
      <div className="max-w-2xl mx-auto px-5 lg:px-8">
        <div className="text-center mb-14">
          ...
          <h2 className="font-black" style={{ color: NAVY, fontSize: 'clamp(1.75rem, 3.5vw, 2.5rem)' }}>{t.howItWorks.heading}</h2>
        </div>
          ...
          {t.howItWorks.steps.map((step, i) => (
            <div key={i} className="flex gap-5">
              <div className="flex flex-col items-center flex-shrink-0">
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center font-black text-white z-10 flex-shrink-0 text-[0.9rem]" style={{ background: GRAD, boxShadow: '0 4px 20px rgba(37,99,235,0.3)', minWidth: '48px' }}>
                  {step.num}
                </div>
        ...
        <div className="text-center mt-10">
          <button onClick={() => scrollTo('contact')} className="inline-flex items-center gap-2 font-bold px-7 py-3 rounded-xl text-sm transition-all text-white hover:shadow-lg" style={{ background: GRAD, boxShadow: '0 4px 20px rgba(37,99,235,0.25)' }}>
            {t.howItWorks.cta}
