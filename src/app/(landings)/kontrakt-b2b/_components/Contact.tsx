...
export default function Contact() {
  ...
  const handleFormFocus = () => {
    if (!formStarted) {
      setFormStarted(true);
      pushEvent(EVENTS.formStart, { landing_slug: 'landing1', segment: 'kontrakt-b2b' });
    }
  };
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        sessionStorage.setItem('fdk_lp_slug', 'landing1');
        sessionStorage.setItem('fdk_lp_segment', 'kontrakt-b2b');
        pushEvent(EVENTS.formSubmit, { landing_slug: 'landing1', segment: 'kontrakt-b2b', event_id: eventId });
        ...
        <GlowCard>
          <form onSubmit={handleSubmit(onSubmit)} noValidate className="p-8 grid grid-cols-1 sm:grid-cols-2 gap-5" onFocus={handleFormFocus}>
            ...
            <div style={{ position: 'absolute', left: '-9999px', top: '-9999px' }} aria-hidden="true">
              <label htmlFor="ws-kb">Website</label>
              <input id="ws-kb" type="text" tabIndex={-1} autoComplete="off" {...register('website')} />
            </div>

            {/* Imię */}
            <div>
              <label htmlFor="fn-kb" className={lbl} style={{ color: NAVY }}>{t.contact.fields.name}<span style={{ color: BLUE }}> *</span></label>
              <input id="fn-kb" type="text" autoComplete="given-name" className={inp} {...register('firstName')} aria-invalid={!!errors.firstName} />
              {errors.firstName && <p className={errCls} role="alert">{errors.firstName.message}</p>}
            </div>

            {/* E-mail */}
            <div>
              <label htmlFor="em-kb" className={lbl} style={{ color: NAVY }}>{t.contact.fields.email}<span style={{ color: BLUE }}> *</span></label>
              <input id="em-kb" type="email" autoComplete="email" className={inp} {...register('email')} aria-invalid={!!errors.email} />
              {errors.email && <p className={errCls} role="alert">{errors.email.message}</p>}
            </div>

            {/* Telefon */}
            <div className="sm:col-span-2">
              <label htmlFor="ph-kb" className={lbl} style={{ color: NAVY }}>{t.contact.fields.phone}</label>
              <input id="ph-kb" type="tel" autoComplete="tel" className={inp} {...register('phone')} />
            </div>

            {/* Opis */}
            <div className="sm:col-span-2">
              <label htmlFor="desc-kb" className={lbl} style={{ color: NAVY }}>{t.contact.fields.description}<span style={{ color: BLUE }}> *</span></label>
              <textarea id="desc-kb" rows={3} placeholder={t.contact.fields.descriptionPlaceholder} className={`${inp} resize-y`} required {...register('description')} aria-invalid={!!errors.description} />
