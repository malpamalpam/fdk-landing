        ...
        <AccordionPrimitive.Root
          ...
          onValueChange={(val) => {
            if (val) {
              ...
              if (q) pushEvent(EVENTS.faqOpen, { landing_slug: 'landing1', question: q.slice(0, 60) });
            }
          }}
        >
          {t.faq.items.map((item, i) => (
            <AccordionPrimitive.Item key={`${lang}-${i}`} value={`item-${i}`} className="bg-white rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(37,99,235,0.1)' }}>
              ...
              <AccordionPrimitive.Content className="overflow-hidden">
                <div className="px-6 pb-5 pt-3 border-t space-y-2.5" style={{ borderColor: 'rgba(37,99,235,0.08)' }}>
                  {item.a.split('\n\n').map((para, pi) => (
                    <p key={pi} className="text-sm leading-relaxed" style={{ color: MID }}>{para}</p>
                  ))}
                </div>
